...
class UnionFind{
    public:
    vector<ll> p, rank, setSize, minele, maxele;
    ll numSets;
    ...
    void UnionSet(ll i, ll j){
        ...
        minele[y] = min(minele[x], minele[y]);
        maxele[y] = max(maxele[x], maxele[y]);
        setSize[y] += setSize[x];
        numSets--;
    }
...
void solve(){
    ...
    while(q--){
        ...
        if(str[0] == 'g'){
            ll x; cin >> x;
            ll use = u.findSet(x);
            cout << u.minele[use] << " " << u.maxele[use] << " " << u.setSize[use] << "\n";
        }
        else{
            ll a, b; cin >> a >> b;
            u.UnionSet(a,b);
